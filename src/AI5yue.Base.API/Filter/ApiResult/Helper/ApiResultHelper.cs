...
namespace AI5yue.Base.API.Filter.ApiResult.Helper
{
    /// <summary>
    /// 用来规范Api返回model
    /// </summary>
    public class ApiResultHelper
    {
        ...
        /// <returns></returns>
        public static ApiResultModel<T> CustomResult<T>(string message, int status)
        {
            var result = new ApiResultModel<T>();
            if (status > -1)
            {
                result.Status = (HttpStatusCode)status;
        ...
        /// <param name="status"></param>
        /// <returns></returns>
        public static ApiResultModel<T> SuccessResult<T>(string message = null, int status = 200)
        {
        ...
        /// <param name="status"></param>
        /// <returns></returns>
        public static ApiResultModel<T> BadResult<T, S>(string message = "Http Request Fail", int status = 400, string userName = "")
        {
            return CustomResult<T>(message, status);
        }
        ...
        /// <param name="status"></param>
        /// <returns></returns>
        public static ApiResultModel<T> ValidationErrorResult<T>(string message = "Validation Error", int status = 400)
        {
            return CustomResult<T>(message, status);
        }
